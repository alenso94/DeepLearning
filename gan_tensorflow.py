...
def generator(x):
    ...
    #o = tf.nn.tanh(tf.matmul(h0, w2) + b2)
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    testimages = sess.run(G_z, {z: z_, drop_out: 0.0})
    sfg = 5
    fig, ax = plt.subplots(sfg, sfg, figsize=(5, 5))
    ...
    for k in range(5*5):
        ...
        j = k % 5
        ax[i, j].cla()
        ax[i, j].imshow(np.reshape(testimages[k], (28, 28)), cmap="gray")
